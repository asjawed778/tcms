...
const SubjectCard: React.FC<SubjectCardProps> = ({
  ...
  return (
    <>
      ...
      >
        {hover && (
          <Box sx={styles.hoverWrapper} onClick={(e) => e.stopPropagation()}>
            <IconButton
              size="small"
              color="primary"
        ...
        <Box className="card-content" sx={{ display: "flex", width: "100%" }}>
          <Box sx={styles.subjectTitleWrapper}>
            <Typography variant="h6" fontWeight={700} sx={styles.subjectTitle}>
              {subject.name}
            </Typography>
          ...
          <Box sx={styles.subjectTypeWrapper}>
...
const getStyles = () => ({
  ...
  subjectTypeWrapper: { flex: 1, p: 1, bgcolor: "#fff", width: 200 },
});
